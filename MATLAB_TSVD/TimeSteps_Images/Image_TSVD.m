...
%% Conversion and printing out results
...
%lambda=0;
mTildaArray = reshape(compute_inverse(lambda,9,UU,SS,VV)*d,size(Geo));
...
%mTildaArray = reshape(compute_inverse(0,256,UU,SS,VV)*d,size(Geo));
 ...
 saveas(gcf,append(dirname,sprintf('rec3DSVD%d00.png',tn)))
 ...
 %   subplot(1,5,tn)
   ...
   % caxis([0 0.8e-7])
    ...
    norm(d,2)
      saveas(gcf,append(dirname,sprintf('recSVD%d00.png',tn)))
end
